...
namespace HGM.Hotbird64.LicenseManager
{
    ...
    public unsafe struct VlmcsdHeader
    {
        ...
        public MemoryStream WriteData(bool includeAppItems, bool includeKmsItems, bool includeSkuItems, bool noText, bool includeBetaSkuItem)
        {
            ...
            var hostBuildSize = HostBuild.Size * HostBuildCount;
            ...
            var hostBuilds = new HostBuild[HostBuildCount];
            var textData = new VlmcsdDataText[idData.Length];
            var currentText = (ulong)idData.Length * VlmcsdData.Size + Size + csvlkDataSize + hostBuildSize;
            var ePids = new VlmcsdDataText[CsvlkCount];
            var iniFileNames = new VlmcsdDataText[CsvlkCount];
            var csvlkNames = new VlmcsdDataText[CsvlkCount];
            var csvlkDataList = new CsvlkData[CsvlkCount];
            var hostBuildNames = new VlmcsdDataText[HostBuildCount];
            ...
            for (var i = 0; i < HostBuildCount; i++)
            {
                hostBuilds[i].Flags =
            ...
            for (var i = 0; i < csvlkDataList.Length; i++)
            {
            ...
            for (var i = 0; i < hostBuilds.Length; i++)
            {
            ...
            {
                foreach (var text in hostBuildNames)
                {
