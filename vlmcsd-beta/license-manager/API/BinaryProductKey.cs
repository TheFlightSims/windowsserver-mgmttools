...
namespace HGM.Hotbird64.Vlmcs
{
    ...
    {
        [FieldOffset(0)]
        private ulong Uint64Low;
        [FieldOffset(8)]
        private ulong Uint64High;
        [FieldOffset(0)]
        private unsafe fixed ulong u64[2];
        [FieldOffset(0)]
        private unsafe fixed uint u32[4];
        [FieldOffset(0)]
        private unsafe fixed ushort u16[8];
        [FieldOffset(0)]
        private unsafe fixed byte u8[16];
        ...
        public bool IsNewKey
        {
            ...
            set
            {
                if (value && !IsNewKey)
                {
                    Uint64High |= 0x8000000000000;
                }
                else if (!value && IsNewKey)
                {
                    Uint64High &= 0xfff7ffffffffffff;
                }
            }
        }
        ...
        public uint Id
        {
            get { ThrowOnOldKey(); return (uint)((Uint64Low >> 20) & 0x3fffffff); }
            set
            {
                ThrowOnOldKey();
        ...
        public ushort StoredCrc32
        {
            get
            {
                ThrowOnOldKey();
                return (ushort)((Uint64High >> 39) & 0x3ff);
            }
        }

        public ushort CalculatedCrc32
        {
            get
            {
                ThrowOnOldKey();
        ...
        public ulong UnknownSecret
        {
            get
            {
                ThrowOnOldKey();
                return (Uint64Low >> 50) | ((Uint64High & 0x7fffffffff) << 14);
            }
            set
            {
                ThrowOnOldKey();
        ...
        public uint Group
        {
            get { ThrowOnOldKey(); return (uint)(Uint64Low & 0xfffff); }
            set
            {
                ThrowOnOldKey();
        ...
        public static unsafe explicit operator string(BinaryProductKey binaryKey)
        {
            ...
            for (int i = 24; i >= 0; i--)
            {
                unchecked
                {
                    ...
                    binaryCodedBase24Key[i] = (byte)current;
                }
            }
        ...
        public static unsafe explicit operator BinaryProductKey(string key)
        {
            ...
            for (i = 0; i < 25; i++)
            {
                ...
                if (isNewKey && i == 0)
                {
                    current = key.IndexOf('N');
                    key = key.Remove((int)current, 1);
                }
                else
                {
        ...
        {
            ThrowOnOldKey(ref binaryKey);

            return
                $"{GetFirstEpidPart()}-{binaryKey.Group:00000}-" +
                $"{binaryKey.Id / 1000000:000}-{binaryKey.Id % 1000000:000000}-" +
                $"{((msKeyType < 0) ? "XX" : $"{msKeyType: 00}")}-" +
                $"{CultureInfo.InstalledUICulture.LCID}-{Environment.OSVersion.Version.Build}.0000-" +
                $"{DateTime.UtcNow.DayOfYear:000}{DateTime.UtcNow.Year:0000}";
        ...
        public static string GetEpid(string key, int msKeyType = -1)
        {
